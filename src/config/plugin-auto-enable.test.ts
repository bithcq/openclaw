...
describe("applyPluginAutoEnable", () => {
  ...
  it("auto-enables wecom plugin when configured via env", () => {
    const result = applyPluginAutoEnable({
      config: {},
      env: {
        WECOM_CORP_ID: "ww123",
      ...
      manifestRegistry: makeRegistry([{ id: "wecom", channels: ["wecom"] }]),
    });

    expect(result.config.plugins?.entries?.wecom?.enabled).toBe(true);
